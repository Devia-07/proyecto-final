...
def fly(window_search=None):
    ...
    # imagen banner
    image_logo = ctk.CTkImage(dark_image=Image.open(
        fr"imagenes\Heaven.png"), size=(200, 700))
    image_logo = ctk.CTkLabel(window_fly, image=image_logo, text="")
    image_logo.place(x=50, y=40)
    see_checkin = ctk.CTkButton(window_fly, text="check in", command=lambda: functions(
        check_in(window_fly)), width=100, height=40)
    ...
    label_passenger = ctk.CTkLabel(
        frame_busqueda, text="Número de pasajeros", text_color="white")
    ...
    imagen_medellin = ctk.CTkImage(
        dark_image=Image.open(imagen_path), size=(300, 100))
    label_imagen_medellin = ctk.CTkLabel(
        frame_medellin, image=imagen_medellin, text="")
    ...
    imagen_bogota = ctk.CTkImage(
        dark_image=Image.open(imagen_path), size=(300, 100))
    label_imagen_bogota = ctk.CTkLabel(
        frame_bogota, image=imagen_bogota, text="")
...
def search_fly(indices, window_fly, peoples):
    ...
    frame_label = ctk.CTkFrame(
        window_search, fg_color="#EBEBEB", border_color="blue", border_width=3)
    ...
    filter = ctk.CTkComboBox(frame_filter, values=[
                             "barato", "medio", "caro"], state="readonly")
    filter.set("precio")
    ...
    for day in range(1, 31):
        ...
        if date.weekday() in [2, 3]:  # 2 es miércoles, 3 es jueves
            dates_june.append(date.strftime("%Y-%m-%d"))
    filter_days = ctk.CTkComboBox(
        frame_filter, values=dates_june, state="readonly")
    filter_days.set("dias")
    # filtrar los dias de junio
    filter_days.grid(row=0, column=2)
    buttons = []
    button_filter = ctk.CTkButton(frame_filter, text="Filtrar", command=lambda: c.filter_search(
        filter.get(), filter_days.get(), indices, window_search, peoples, buttons, frame_buttons))
...
def info_buy(indice, window, indices, peoples):
    ...
    frame_hours_f = ctk.CTkFrame(window_info, fg_color="white",
                                 border_color="blue", border_width=3, width=1000, height=300)
    frame_hours_f.place(x=250, y=200)

    label_hours = ctk.CTkLabel(
        frame_hours_f, text="Vuelos disponibles", font=("Times New Roman", 20))
    label_hours.grid(row=0, column=0, pady=(10, 0))

    scroll_hours = ctk.CTkScrollableFrame(
        frame_hours_f, width=1000, height=300, fg_color="white")
    ...
    for i in hours:
        ...
        button_hours.grid(row=row_counter, column=0,
                          padx=(350, 0), pady=10, sticky="EW")
        row_counter += 1
    ...
    # y=-10 to give some padding from the bottom
    button_back.place(relx=0.4, rely=0.9, anchor='s', y=-10)
...
def buy(indice, window, indices, peoples):
    ...
    label_text = ctk.CTkLabel(
        window_buy, text="Selecciona alguno de los paquetes", font=("Times New Roman", 30))
    label_text.place(x=550, y=100)

    # Initialize frames with specified width and increased height
    frame_1 = ctk.CTkFrame(window_buy, border_color="blue",
                           border_width=3, height=1700, width=500)
    frame_2 = ctk.CTkFrame(window_buy, border_color="blue",
                           border_width=3, height=1700, width=500)
    frame_3 = ctk.CTkFrame(window_buy, border_color="blue",
                           border_width=3, height=1700, width=500)
    ...
    button_aluminio = ctk.CTkButton(frame_1, text="Comprar", width=100, height=60, command=lambda: fc.aluminum(
        indice, window_buy, peoples, df["ValorMin"].values[indice]))
    ...
    button_diamond = ctk.CTkButton(frame_2, text="Comprar", width=100, height=60, command=lambda: fc.diamond(
        indice, window_buy, peoples, df["ValorMedio"].values[indice]))
    ...
    button_premium = ctk.CTkButton(frame_3, text="Comprar", width=100, height=60, command=lambda: functions(
        choose_sit(indice, window_buy, indices, peoples)))
...
def record_check_in(window, peoples, indice, sits, price):
    ...
    label_fly = ctk.CTkLabel(
        frame_info_fly, text="Información de vuelo", font=("Arial", 20))
    label_fly.place(x=250, y=50)
    label_fly1 = ctk.CTkLabel(frame_info_fly, text=f"Vuelo: {
                              df['Vuelo'].values[indice]}", font=("Arial", 16))
    label_hour = ctk.CTkLabel(frame_info_fly, text=f"Hora de salida: {
                              df['HoraSalida'].values[indice]}", font=("Arial", 16))
    label_hour1 = ctk.CTkLabel(frame_info_fly, text=f"Hora de llegada: {
                               df['HoraLlegada'].values[indice]}", font=("Arial", 16))
    label_price = ctk.CTkLabel(frame_info_fly, text=f"Precio total: {
                               price}", font=("Arial", 16))
    label_price_unitario = ctk.CTkLabel(frame_info_fly, text=f"Precio unitario: {
                                        price/peoples}", font=("Arial", 16))
    ...
    for i in range(peoples):
        ...
        radio_si = ctk.CTkRadioButton(
            frame_persona, text="Sí", variable=asistencia, value="Sí")
...
def tickets(window, peoples, datas, sits, indice):
    ...
    frame_scroll = ctk.CTkScrollableFrame(
        window_tickets, width=600, height=300)
    frame_scroll.grid(row=0, column=0)
    combination = fc.randomiser(peoples, indice)
    for i in range(peoples):
        frame_ticket = ctk.CTkFrame(
            frame_scroll, fg_color="White", border_color="green", border_width=2)
        frame_ticket.grid(row=i, column=0, padx=10, pady=10)
        image_ticket = ctk.CTkImage(dark_image=Image.open(
            "imagenes/ticket_vuelo.png"), size=(600, 300))
        label_ticket = ctk.CTkLabel(frame_ticket, image=image_ticket, text="")
        label_ticket.grid(row=0, column=0)

        label_name = ctk.CTkLabel(frame_ticket, text=f"Nombre: {datas[i][1]} {
                                  datas[i][2]}", text_color="black")
        label_name.place(x=98, y=100)
        label_origin = ctk.CTkLabel(
            frame_ticket, text=f"{df['CiudadOrigen'].values[indice]}", text_color="black")
        label_origin.place(x=98, y=170)
        label_destiny = ctk.CTkLabel(
            frame_ticket, text=f"{df['CiudadDestino'].values[indice]}", text_color="black")
        label_destiny.place(x=98, y=235)
        label_hour = ctk.CTkLabel(frame_ticket, text=f"{
                                  df['HoraSalida'].values[indice]}", text_color="black")
        label_hour.place(x=380, y=235)
        label_hour1 = ctk.CTkLabel(frame_ticket, text=f"{
                                   df['HoraLlegada'].values[indice]}", text_color="black")
        label_hour1.place(x=450, y=235)
        label_fly = ctk.CTkLabel(frame_ticket, text=f"{
                                 df['Vuelo'].values[indice]}", text_color="black")
        label_fly.place(x=230, y=175)
        label_code = ctk.CTkLabel(frame_ticket, text=f"Código de abordaje: {
                                  combination[i]}", text_color="black")
        label_code.place(x=98, y=265)
        label_date = ctk.CTkLabel(frame_ticket, text=f"{
                                  df['Fecha'].values[indice]}", text_color="black")
        label_date.place(x=330, y=175)
    button = ctk.CTkButton(window_tickets, text="siguiente",
                           command=lambda: return_menu(window_tickets))
...
def check_in(window):
    ...
    button = ctk.CTkButton(frame_check_in, text="Realizar Check-in",
                           command=lambda: c.condition_check_in(entry_code.get(), entry_dni.get(), window_check_in))
...
def admin(window_check):
    ...
    ctk.set_appearance_mode("light")
    button_see_profits = ctk.CTkButton(
        window_admin, text="Ver ganancias", command=lambda: c.see_profits())
    button_see_profits.place(relx=0.5, rely=0.5, anchor="center")
    button_back = ctk.CTkButton(
        window_admin, text="Volver", command=lambda: return_menu(window_admin))
