...
def fly(window_search=None):
    ...
    origen, destino, fecha = c.flight_list()
...
def choose_sit(index=None, window=None, indexs=None):
    if window is not None:
        window.destroy()
...
def tickets(window, peoples, datas, sits, index):
    ...
    for i in range(peoples):
            ...
            combination = fc.randomiser(peoples, index)
            ...
            label_code = ctk.CTkLabel(frame_ticket, text=f"Código de abordaje: {combination}")
