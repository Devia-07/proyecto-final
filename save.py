...
def record_base(peoples, datas, sits, indice, window, pay_client):
    ...
    for i in range(peoples):
        ...
        base_datos = {"nombre": [datas[i][1]], "apellido": [datas[i][2]], "dni": [
            datas[i][3]], "telefono": [datas[i][6]], "email": [datas[i][5]]}
        df1 = pd.DataFrame(dataframe)
        df2 = pd.DataFrame(base_datos)
        if os.path.isfile("clientes.csv"):
            df2.to_csv("clientes.csv", mode='a',
                       header=False, index=False, sep=";")
        else:
            df2.to_csv("clientes.csv", index=False,
                       sep=";", header=True, mode='a')

        if not os.path.isfile(f'{vuelo}.csv'):
            df1.to_csv(f'{vuelo}.csv', index=False,
                       sep=",", header=True, mode='a')
        else:
            df1.to_csv(f'{vuelo}.csv', mode='a',
                       header=False, index=False, sep=",")
...
def search_low(filter, day, indices, window, peoples, buttons, frame):
    ...
    # Filtra el DataFrame con los índices
    df_filtered = df_filtered[df_filtered['Fecha'] == day]
    if df_filtered.empty:
        mb.showerror("error", "no hay vuelos disponibles ese dia")
        return
    df_filtered = df_filtered.sort_values(
        by='ValorMin', ascending=True)  # Ordena el DataFrame filtrado
    rows = 0
    for i in df_filtered.index:  # Itera sobre los índices del DataFrame filtrado y ordenado
        ...
        button = ctk.CTkButton(frame, text=mensaje, command=lambda i=i: mn.info_buy(
            i, window, indices, peoples), width=900, height=150, font=("Arial", 20))
...
def search_medium(filter, days, indices, window, peoples, buttons, frame):
    ...
    # Filtra el DataFrame con los índices
    df_filtered = df_filtered[df_filtered['Fecha'] == days]
    df_filtered = df_filtered.sort_values(
        by='ValorMedio', ascending=True)  # Ordena el DataFrame filtrado
    rows = 0
    if df_filtered.empty:
...
def search_high(filter, days, indices, window, peoples, buttons, frame):
    ...
    df_filtered = df_filtered[df_filtered['Fecha'] == days]
    df_filtered = df_filtered.sort_values(
        by='ValorMax', ascending=True)  # Ordena el DataFrame filtrado
    rows = 0
    if df_filtered.empty:
        mb.showerror("error", "no hay vuelos disponibles")
        return
...
def record_profits(name, number, date, code, price):
    if os.path.isfile("profits.csv"):
        data = {"nombre": [name], "numero": [number], "fecha": [
            date], "codigo": [code], "precio": [price], "ganancia": [0]}
